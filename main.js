...
function connectWebSocket() {
  ...
  socket.onmessage = (event) => {
    try {
      ...
      let rawAngle = parseFloat(data.values?.[0]);

      if (!isNaN(rawAngle)) {
        if(rawAngle < 0) rawAngle = 0
        phoneAngle = rawAngle * (Math.PI / 2);
        
        console.log("Data: ", data);
        console.log("Angle: ", phoneAngle * (180 / Math.PI));
      } else {
...
function draw() { 
    ...
    let rotateByPhone = m4.axisRotation([0,0,1], phoneAngle)
    ...
    rotatedModelView = m4.multiply(rotateByPhone, modelView)
    matAccum0 = m4.multiply(rotateToPointZero, rotatedModelView );
